...
Support (use whenever needed): Slides • Answer Keys • Prompt Templates.

Rule: ONLY move down when you can explain the current step in your own words.

Key ideas:
- Move clockwise; don’t rush to puzzles.
- Reflection (teaching it back) locks in learning.
- Mastery check loops you back only where needed (targeted review, not full restart).
- Support artifacts (slides, keys, prompts) assist mentors without taking away student discovery.
...
Maintenance Tip: When adding a new concept, add (a) at least one guided example, (b) a homework entry or confirm coverage in an existing PDF, (c) a puzzle that reuses 70–80% of the just-learned cognitive pattern (not brand-new ideas), (d) update this table.
...
Want to help make MathCodeLab better? We welcome contributions! Please open an issue (typo, suggestion) or send a PR (new example, clarification, puzzle).
