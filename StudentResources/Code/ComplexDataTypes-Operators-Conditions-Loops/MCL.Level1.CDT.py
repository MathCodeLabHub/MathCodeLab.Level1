...
This file focuses on complex data types such as lists, tuples, ranges,
dictionaries and sets.  It shows how to work with these types,
including creating them, iterating over them, using membership and
identity operators, and combining nested loops.  Each code example is
wrapped in triple quotes (single quotes); remove the quotes to execute.

1. Complex built-in data types – examples of lists, tuples, ranges,
   dictionaries and sets.
2. Converting between types – converting sequences of strings to
   numbers.
...
6. More range patterns – converting ranges to lists and counting
   backwards.
7. Membership and identity operators – using `in`, `not in`, `is` and
   `is not`.
'''
