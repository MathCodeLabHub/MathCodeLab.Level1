...
```python
# Data Type Detective program using string methods
user_input = input("Enter something and I'll guess its type: ")

# Handle negative numbers by checking for minus sign
if user_input.startswith('-'):
    check_input = user_input[1:]  # Remove the minus sign
    has_negative = True
else:
...
# Check if it's a float (has exactly one decimal point)
elif '.' in check_input and check_input.count('.') == 1:
    # Split by decimal point and check both parts
...
# Check if all characters are digits (or digits with minus sign)
